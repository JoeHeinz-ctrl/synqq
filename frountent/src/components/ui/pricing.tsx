...
export function Pricing({
  ...
  return (
    <div className="container py-20">
        ...
        <label className="relative inline-flex items-center cursor-pointer">
          <Label>
            <Switch
              ...
              checked={!isMonthly}
              onCheckedChange={(v: any) => handleToggle(Boolean(v))}
              className="relative"
            />
          </Label>
        </label>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 sm:2 gap-6" style={{ perspective: 1200 }}>
          ...
          return (
            <motion.div
              ...
              whileInView={
                isDesktop
                  ? {
                      y: plan.isPopular || isCenter ? -28 : -6,
                      opacity: 1,
                      x: index === 0 ? -22 : index === 2 ? 22 : 0,
                      scale: isCenter ? 1.03 : plan.isPopular ? 1.01 : 0.99,
                      rotateY: isCenter ? 0 : index === 0 ? 6 : index === 2 ? -6 : 0,
                    }
                  : { y: 0, opacity: 1, x: 0, scale: 1 }
              }
              viewport={{ once: true }}
              transition={{ duration: 0.9, type: "spring", stiffness: 80, damping: 18, delay: 0.18 + index * 0.06 }}
